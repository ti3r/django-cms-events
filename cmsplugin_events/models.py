...
class MonthEventManager(models.Manager):
    ...
    def find(self, month=datetime.date.today().month):
        ...
        return super(MonthEventManager, self).get_queryset() \
            ...
            .filter(event_start__lte=month_end)

    def find_by_category(self, category ,month=datetime.date.today().month):
        n = datetime.date.today()
        month_start = datetime.date(n.year, month, 1)
        range = monthrange(n.year, month)
        month_end = datetime.date(n.year, month, range[1])
...
MONTH_FILTER_CHOICES = ((1, _('January')), (2, _('February')), (3, _('March')), (4, _('April')), (5, _('May')),
...
class EventRegistryPlugin(CMSPlugin):
    ...
    send_message = models.CharField(verbose_name=_('Submit Text'), help_text=_('Text to display in submit button'),
    ...
    category_filter = models.ForeignKey(Category, null=True, blank=True, verbose_name=_('Categories Filter'),
                                        help_text=_('Only events in this category will be displayed as available for '
                                                    'registration'))
    month_filter = models.PositiveIntegerField(verbose_name=_('Month Filter'),
                                               help_text=_('Allow registration for events only on this month'),
                                               null=True, blank=True, choices=MONTH_FILTER_CHOICES)
    current_month_filter = models.NullBooleanField(verbose_name=_('Current Month Filter'),
                                            help_text=_('Allow registration for events only on the current month'),
